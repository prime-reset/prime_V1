...
export default function SettingsPage() {
  ...
  const planLabel = useMemo(() => {
    ...
    if (plan === "founder") return "PRIME Founder";
    if (plan === "standard") return "PRIME Standard";
  ...
  const statusLabel = useMemo(() => {
    ...
    if (subscriptionStatus === "unpaid") return "Impayé";
    return "En attente";
  ...
  async function loadSettings() {
    try {
      ...
      if (profileError) {
        console.error("[PRIME Settings] Profile:", profileError);
      }
      ...
      if (subscriptionError) {
        console.error("[PRIME Settings] Subscription:", subscriptionError);
        setPlan(null);
        setSubscriptionStatus(null);
        return;
      }
      ...
      console.error("[PRIME Settings] Load:", error);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleManageSubscription() {
    if (role === "super_admin") {
      setMessage("Ton accès Super Admin ne nécessite aucun abonnement Stripe.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.error || "Impossible d’ouvrir le portail Stripe."
        );
      }

      if (!data?.url) {
        throw new Error("Stripe n’a retourné aucune URL.");
      }

      window.location.href = data.url;
    } catch (error) {
      console.error("[PRIME Billing Portal]", error);
      setMessage(
        error?.message ||
      ...
      <div className="page">
        ...
        <section className="section">
          ...
          <div className="info-grid">
            ...
            <InfoRow label="Statut" value={statusLabel} success={subscriptionStatus === "active" || subscriptionStatus === "trialing" || role === "super_admin"} />
            <InfoRow
              label="Facturation"
              value={
                role === "super_admin"
...
function SettingAction({ icon, title, subtitle, href, onClick, danger, disabled = false }) {
  const content = (
    <>
